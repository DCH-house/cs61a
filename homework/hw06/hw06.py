...
class VendingMachine:
    ...
    def restock(self, stock):
        self.item_cnt += stock
        print(f'\'Current {self.item} stock: {self.item_cnt}\'')
...
def deep_map_mut(func, lnk):
    ...
    while current_node is not Link.empty:
        if isinstance(current_node.first, Link):
            deep_map_mut(func, current_node.first)
        else:
            current_node.first = func(current_node.first)
...
def two_list(vals, counts):
    ...
    for i in range(len(vals)):
        j = counts[i]
        while j > 0:
            new_node = Link(vals[i], tail.rest)
            tail.rest = new_node
            tail = tail.rest
            j -= 1
