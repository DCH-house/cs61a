...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.is_double_damage == False:
            if self.is_container:
                self.damage = 2 * self.damage
                self.double(self.store_ant)
            else:
                self.damage = 2 * self.damage
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not position.is_hive:
            if position.bees != None and len(position.bees) > 0 and distance >= self.lower_bound and distance <= self.upper_bound:
...
# The HungryAnt Class
class HungryAnt(Ant):
    '''HungryAnt will eat a bee at its place, and takes 3 turns to chew, during this period, the Ant 
       do nothing
    '''
    name = 'Hungry'
    damage = 0
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0 and self.place.bees != None and len(self.place.bees) > 0:
...
class TankAnt(ContainerAnt):
    '''
    The TankAnt is a ContainerAnt that protects an ant in its place and also deals 1 damage to all bees 
    in its place each turn. Like any ContainerAnt, a TankAnt allows the ant that it contains to perform 
