...
def boar_brawl(player_score, opponent_score):
    ...
    return max(1, 3 * abs(ten_digit - one_digit))
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    cnt_factors = num_factors(score)
    if cnt_factors == 3 or cnt_factors == 4:
        temp = score + 1
        while True:
            if is_prime(temp):
                return temp
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    get_score = boar_brawl(score, opponent_score)
    if get_score >= threshold:
        return 0
