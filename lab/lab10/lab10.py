...
def floor_div(args):
    ...
    dividend = args.first
    pair_rest = args.rest
    divisor = pair_rest.first

    if isinstance(dividend, Pair):
        dividend = calc_eval(dividend)
    if isinstance(divisor, Pair):
        divisor = calc_eval(divisor)

    if pair_rest.rest == nil:
        return dividend // divisor
    pair_rest.first = dividend // divisor
    return floor_div(pair_rest)
...
def eval_and(expressions):
    ...
    if isinstance(first, Pair):
        first = calc_eval(first)

    if first is scheme_f:
        return False
    second = expressions.rest
    if second is nil:
        return first
    
    return eval_and(second)
...
def eval_define(expressions):
    ...
    if isinstance(symbol, Pair):
        symbol = calc_eval(symbol)
    if isinstance(value, Pair) or isinstance(value, str):
        value = calc_eval(value)
